...
function CrudApp() {
  ...
  const updateStudent = async (id) => {
    if (editingStudent.name && editingStudent.rollno && editingStudent.age && editingStudent.email) {
      const response = await axios.put(`http://localhost:3000/students/${id}`, editingStudent);
  ...
  const searchByRollno = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/students/search/${searchRollno}`);
